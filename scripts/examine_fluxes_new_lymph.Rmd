...
contour_df %>% 
  ...
  # geom_point() +
  labs(x = "Production of R2 in moles/cm^3 tissue/s", y = "V165 Secretion in moles/cm^3 tissue/s",
       title = "Contour plot of Percent Bound R2 and Free V165",
        subtitle = "Log Scale") +
  scale_x_log10() + scale_y_log10()

```
In addition to contours of the amount of bound R2:
```{r}
contour_df %>% 
  ggplot(aes(kprod_R2, q_V165  * 1534 / (1e-5 * 6.023e23))) + 
  geom_point() +
...
<!-- contour_df %>%  -->
<!--   ggplot(aes(kprod_R2, q_V165  * 1534 / (1e-5 * 6.023e23))) +  -->
<!--   stat_contour(aes(z = log(free_V165), color = as.factor(round(exp(..level..)))), bins= 10) + -->
<!--   labs(x = "Production of R2 in moles/cm^3 tissue/s", y = "V165 Secretion in moles/cm^3 tissue/s", -->
...
<!-- ``` -->
## Contour plots for ratio of R2 concentration with ligand vs no ligand:
```{r}
...
contour_df2 %>% 
  ...
  scale_color_gradient2("Total R2 (#/cell)", 
                        ...
                        labels = function(breaks) round(exp(breaks))) +
  labs(x = "Production of R2 in moles/cm^3 tissue/s", y = "V165 Secretion in moles/cm^3 tissue/s",
       title = "Contour plot of Total R2 with no Ligand", 
        subtitle = "Log Scale",
       lty = "Total R2 near 3000") +
  scale_x_log10() + scale_y_log10()
...
contour_df2 %>% 
  ...
  new_scale("color") +
  geom_contour(aes(z = log(free_V165.x), color = after_stat(level)),
               bins = 10) +
  scale_color_gradient("Free V165 (pM)",
                       low = "#e8cced", high = "#762A83",
                       breaks = log(c(0,1,10,100,1000,10000,100000)),
  ...
  # geom_point() +
  labs(x = "Production of R2 in moles/cm^3 tissue/s", y = "V165 Secretion in moles/cm^3 tissue/s",
       title = "Contour plot of Free V165 and the Ratio of Total R2\nfrom Model with Ligand to Model Without Ligand", 
       subtitle = "Log Scale") +
  scale_x_log10() + scale_y_log10()
```
